...
@section('content')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            ...
            <div class="hidden md:block mx-auto text-slate-500" id="pagination-info">
                Showing {{ $promotions->firstItem() }} to {{ $promotions->lastItem() }} of {{ $promotions->total() }}
                entries
            </div>
            <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
                <form method="GET" action="{{ route('admin.promotion.index') }}" id="searchForm">
        ...
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            <table class="table table-report -mt-2" id="promotionTable">
                <thead>
                    <tr>
                        ...
                        <tr class="intro-x">
                            <td>
                                <a href="{{ route('admin.promotion.edit', $promotion->id) }}"
                                    class="font-medium whitespace-nowrap">{{ $promotion->title }}</a>
                            </td>
                            <td class="text-center ">
                                <img src="{{ $promotion->image }}" alt="{{ $promotion->title }}" style="width: auto; max-width: 140px; height: 70px; ">
                            </td>
                            <td class="text-center">
                                {{ $promotion->views }}
                            </td>
                            ...
                            <td class="w-40">
                                <div
                                    class="flex items-center justify-center {{ $promotion->status == 'active' ? 'text-success' : 'text-danger' }}">
